...
class GCN(tf.keras.models.Sequential):
    ...
    def __init__(self, renormalized_matrix, num_classes):
        super().__init__([
            tf.keras.layers.Dropout(0.5),
            GraphConvolution(renormalized_matrix, num_filters=16, activation="relu"),
            tf.keras.layers.Dropout(0.5),
    ...
    def train(self, X, y, epochs):
        for epoch in tqdm(range(epochs)):
            ...
            tqdm.write("epoch: {}/{}, loss: {:.4f}, accuracy: {:.4f}".format(epoch, epochs, loss, float(GCN.accuracy_mask(y, y_pred))))
