...
class Planetoid(tf.keras.Model):
    ...
    def train(self, features, labels, mask_train, mask_val, mask_test, epochs, L_s, L_u, optimizer_u, optimizer_s, train_accuracy, 
        ...
        for epoch in range(1, epochs+1):
            ...
            loss_s, loss_u = self.train_step(features, labels, mask_train, mask_test, L_s, L_u, optimizer_u, optimizer_s,
                                train_accuracy, train_loss, train_loss_u, T1, T2, N1, N2)

            if verbose > 0: print("Train Loss: s {:.3f} u {:.3f}, Train Accuracy: {:.3f}".format(loss_s, loss_u, train_accuracy.result()))
...
class Planetoid_T(Planetoid):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # # Embedding layers for graph context
        # self.embedings = tf.keras.layers.Embedding(self.features_size, self.embedding_size)
    ...
    def eval(self, features, labels, mask, L_s, test_accuracy, test_loss):
        ...
        # test_loss(loss)
        acc = test_accuracy(labels[mask], predictions)
...
class Planetoid_I(Planetoid):
    ...
    def __init__(self, mask_test, *args, **kwargs):
        ...
        # # Parametric Embedding for graph context
        # self.emb_nonfree = tf.keras.layers.Dense(self.embedding_size, activation=tf.nn.relu, kernel_initializer=tf.keras.initializers.GlorotUniform)
    ...
    def call(self, inputs, modality="s"):
        ...
        """
        if modality == "s":
            # W = self.par_embedding.get_weights()
            # self.emb_nonfree.set_weights(W)
            ...
            # freeze some layers 
            self.h_k.trainable = self.h_l.trainable = self.pred_layer.trainable =  False#self.emb_nonfree.trainable =
            self.emb_cont.trainable = True
